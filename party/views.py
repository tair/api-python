...
class PartyOrgCRUD(GenericCRUDView):
    ...
    def get(self, request, format=None):
        ...
        if partnerId is None:
            return HttpResponse("Error. partnerId not provided")
        try:
            cursor = connection.cursor()
            sqlStatement = 'select p.partyId, p.name, (s.endDate>= NOW()) as subscribed from Party p, Subscription s where p.partyId in (\
            ...
            and s.partyId = p.partyId and s.partnerId = "'+ partnerId +'" and (s.endDate>= NOW()) is true'
