#!/usr/bin/env python3
"""
Filter bulk_party_ip_check_error.xlsx for review: same logic as
filter_nstl_ip_check_error_overlap_only.py, but column A is **party id** (not serial id).

Dependencies: CPython 3 + openpyxl only — no Django.
...
def main():
    ...
    default_in = os.path.join(root, "bulk_party_ip_check_error.xlsx")
    default_out = os.path.join(root, "bulk_party_ip_check_error_overlap_only.xlsx")

    p = argparse.ArgumentParser(description=__doc__.split("\n\n")[0])
    p.add_argument("--input", default=default_in, help="Full error workbook from BulkPartyLoadIPCheck")
    ...
    while idx < n:
        ...
        if (
            ...
            if norm_party_id(row[0]) == norm_party_id(row2[0]):
