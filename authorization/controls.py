...
class Authorization:
    @staticmethod
    def getAccessStatus(loginKey, ip, partyId, url, partnerId, hostUrl):
        # check login
        if not Authorization.authentication(loginKey, partyId, url, partnerId, hostUrl):
            return Status.needLogin

        # no login required or is already logged in, check subscription
        if not Authorization.subscription(ip, partyId, url, partnerId, hostUrl):
